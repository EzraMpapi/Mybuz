...
import { IS_CONFIGURED, sb } from "./supabase.jsx";
...
export async function signDocument({ docType, docRef, signerName, content }) {
  ...
  if (IS_CONFIGURED) {
    try { await sb("digital_signatures").insert({ doc_type: docType, doc_ref: docRef, signer_name: signerName, content_hash: contentHash, biometric }).run(); } catch (_e) { notify("Signature reco[...] "); }
  }
  return record;
}
