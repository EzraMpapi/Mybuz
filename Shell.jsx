...
export function SmartManager() {
  ...
  return (
    ...
    <div className={`h-screen w-full flex text-slate-800 overflow-hidden relative text-size-${textSize} ${darkMode ? "dark bg-[#0F172A]" : "bg-[#F8FAFC]"} ${highContrast ? "high-contrast" : ""}`} style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
      <div className="pointer-events-none fixed inset-0 z-0" style={{ background: "radial-gradient(1100px circle at 15% -10%, rgba(22,163,74,0.07), transparent 55%), radial-gradient(900px circle at 100% 0%, rgba(17,24,39,0.05), transparent 50%)" }} />
      <div className="fixed top-0 left-0 right-0 h-[3px] z-[70]" style={{ background: "linear-gradient(90deg, #111827 0%, #16A34A 50%, #22C55E 100%)" }} />

      <Toasts />
      <ConfirmDialog />
      <SendReceiptPanel />
      <PostCreateDispatch company={company} crm={crm} />
      <DailyBriefing company={company} currentUser={currentUser} canManage={canManage} invoices={invoices} inventory={inventory} expenses={expenses} crm={crm} employees={employees} leaveRequests={leaveRequests} workOrders={workOrders} subscriptions={subscriptions} smartAlerts={smartAlerts} enabledModules={enabledModules} />

      {sidebarOpen && (
        <div className="fixed inset-0 bg-slate-900/40 z-40" style={{ animation: "fadeIn .15s ease-out" }} onClick={() => setSidebarOpen(false)} />
      )}

      <aside className={`fixed z-50 h-full w-[240px] shrink-0 flex flex-col bg-white transition-transform duration-200 ease-out overflow-hidden ${darkMode ? "dark-shell" : ""} ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`} style={{ boxShadow: "4px 0 24px rgba(17,24,39,.06)" }}>
        <div className="relative px-5 py-5 border-b border-[#F3F4F6] flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 flex items-center justify-center shrink-0" style={{ background: "linear-gradient(135deg, #22C55E, #16A34A)", clipPath: "polygon(25% 0%, 75% 0%, 100% 50%, 75% 100%, 25% 100%, 0% 50%)", boxShadow: "0 4px 14px rgba(22,163,74,.35)" }}>
              <span className="text-white text-[15px] font-bold" style={{ fontFamily: "'Poppins'" }}>S</span>
            </div>
            <div className="flex flex-col leading-tight">
              <span className="text-[14.5px] font-semibold tracking-tight brand-wordmark" style={{ fontFamily: "'Poppins'" }}>Smart Manager</span>
          ...
          {visibleModules.map((m) => {
            ...
            return (
              <button
                ...
                className={`relative w-full flex items-center justify-between gap-2.5 pl-3 pr-3 py-2.5 rounded-lg text-[13.5px] transition-all duration-200 group ${isActive ? "font-medium" : "text-slate-500 hover:text-[#111827]"}`}
              ...
              >
                {isActive && <span className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-4 rounded-full" style={{ background: "#16A34A" }} />}
        ...
        <div className="relative px-4 py-4 border-t border-[#F3F4F6]">
          <button onClick={() => go("settings")} className={`w-full flex items-center justify-between gap-2.5 text-[13px] transition-colors group ${active === "settings" ? "font-medium" : "text-slate-500 hover:text-[#111827]"}`} style={active === "settings" ? { color: "#16A34A" } : undefined}>
            <span className="flex items-center gap-2.5">
      ...
      <div className="relative z-10 flex-1 flex flex-col min-w-0 w-full">
        <header className={`h-16 shrink-0 bg-white border-b border-slate-200/80 flex items-center justify-between px-4 sm:px-6 ${darkMode ? "dark-shell" : ""}`}>
          <div className="flex items-center gap-3">
            <button className="text-slate-500 hover:text-[#111827] hover:bg-slate-100 rounded-lg p-1.5 -ml-1.5 transition-colors" onClick={() => setSidebarOpen(true)} aria-label="Open menu">
              <MenuIcon />
            </button>
            <div className="flex items-center gap-2 text-[13px] text-slate-500">
              ...
              <span className="hidden md:inline-flex items-center text-[10.5px] font-medium text-slate-400 bg-slate-100 px-2 py-0.5 rounded-full ml-1">{currentUser.role}</span>
            </div>
          </div>
          <div className="flex items-center gap-3 sm:gap-4">
            <span className="hidden sm:flex items-center gap-1.5 text-[11px] font-medium px-2.5 py-1 rounded-full" style={IS_CONFIGURED ? { backgroundColor: "#16A34A14", color: "#16A34A" } : { backgroundColor: "#F59E0B14", color: "#F59E0B" }} title={IS_CONFIGURED ? "Connected to Supabase" : "Running on built-in demo data — connect Supabase to persist changes"}>
            ...
            <button onClick={() => setPaletteOpen(true)} className="flex items-center gap-1.5 text-[12px] font-medium text-slate-400 border border-slate-200 rounded-lg px-2.5 py-1.5 hover:border-slate-300 hover:text-slate-600 transition-colors" aria-label="Search everything">
              <Search size={13} />
        ...
        {active !== "ai" && visibleModules.some((m) => m.id === "ai") && (
          <button onClick={() => go("ai")} aria-label="Open AI Command Center" className="fixed bottom-6 right-6 z-40 w-14 h-14 rounded-full shadow-lg flex items-center justify-center text-white hover:scale-105 transition-transform" style={{ background: "linear-gradient(135deg, #16A34A, #22C55E)" }}>
            <Sparkles size={22} />
          </button>
        )}

        <main key={active} className="module-fade flex-1 overflow-y-auto p-4 sm:p-6 pb-24 sm:pb-6">
          {active === "dashboard" && <Dashboard company={company} invoices={invoices} inventory={inventory} crm={crm} expenses={expenses} leaveRequests={leaveRequests} workOrders={workOrders} subscriptions={subscriptions} employees={employees} posTransactions={posTransactions} currentUser={currentUser} onQuickAction={goWithIntent} onNavigate={go} />}
          ...
          {active === "analytics" && <Analytics company={company} invoices={invoices} expenses={expenses} crm={crm} inventory={inventory} employees={employees} leaveRequests={leaveRequests} workOrders={workOrders} posTransactions={posTransactions} onNavigate={go} />}
          {active === "notifications" && <Notifications inventory={inventory} invoices={invoices} expenses={expenses} leaveRequests={leaveRequests} workOrders={workOrders} subscriptions={subscriptions} canManage={canManage} currentUser={currentUser} smartAlerts={smartAlerts} onNavigate={go} />}
          {active === "activity" && <ActivityStream currentUser={currentUser} />}
          {active === "integrations" && <Integrations invoices={invoices} expenses={expenses} canManage={canManage} currentUser={currentUser} onNavigate={go} />}
          {active === "workflows" && <WorkflowStudio company={company} invoices={invoices} expenses={expenses} inventory={inventory} />}
          {active === "collaboration" && <CollaborationHub currentUser={currentUser} filesHook={files} employees={employees} invoices={invoices} crm={crm} workOrders={workOrders} leaveRequests={leaveRequests} onNavigate={go} />}
          ...
          {active === "ai" && <AIAssistant company={company} invoices={invoices} inventory={inventory} crm={crm} expenses={expenses} employees={employees} leaveRequests={leaveRequests} suppliers={suppliers} quotations={quotations} scheduledWorkflows={scheduledWorkflows} />}
          ...
          {active === "employee-portal" && <EmployeePortal currentUser={currentUser} company={company} employees={employees} leaveRequests={leaveRequests} canManage={canManage} />}
          {active === "settings" && <SettingsPage company={company} setCompany={setCompany} enabledModules={enabledModules} onToggleModule={toggleModule} currentUser={currentUser} setCurrentUser={setCurrentUser} canManage={canManage} darkMode={darkMode} toggleDarkMode={toggleDarkMode} textSize={textSize} onSetTextSize={setTextSize} highContrast={highContrast} onToggleHighContrast={() => setHighContrast((h) => !h)} exportData={{ crm, invoices, expenses, inventory, employees, posTransactions, suppliers }} />}
          {!["dashboard", "crm", "sales", "inventory", "finance", "hr", "manufacturing", "settings", "ai", "reports", "scm", "ecommerce", "documents", "marketing", "pos", "procurement", "projects", "support", "analytics", "notifications", "integrations", "workflows", "collaboration"].includes(active) && <ComingSoon label={MODULES.find((m) => m.id === active)?.label} />}
        </main>
      </div>
    </div>
    </>
